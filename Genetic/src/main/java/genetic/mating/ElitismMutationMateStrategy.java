...
public class ElitismMutationMateStrategy implements MateStrategy {
    ...
    public <A> void mateStrategy(Genetic<A> alg, SelectionStrategy selection, Population<A> population, Population<A> buffer, Population<A> parentsPool, boolean isInLocalOptimum) {
        ...
        selection.populateParentsPool(population, parentsPool, rand);
        for (int i = elites; i < popSize; i++) {
            A parent1 = selection.chooseParent(parentsPool, rand);
            A parent2 = selection.chooseParent(parentsPool, rand);
