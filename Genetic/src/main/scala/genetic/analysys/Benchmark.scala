...
import genetic.genetic.{Genetic, GeneticAlg}
...
case class BenchmarkResult[A](genetic: Genetic[A], attempts: Int, numSuccesses: Int, successes: Array[(Double, Gene[A], Int)], failures: Array[(Double, Gene[A], Int)]) {
  val bestScore: Double = if(successes.length > 0) successes.map(res => genetic.score(res._2)).max else 0
  val successRate: Double = numSuccesses.toDouble / attempts
  val avgSuccessTimeMs: Double = if(successes.length > 0) Util.avgBy(successes)(_._1) * 1000 else Double.PositiveInfinity
  val avgSuccessIterations: Double = if(successes.length > 0) Util.avgBy(successes)(_._3) else Double.PositiveInfinity
