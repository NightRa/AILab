...
import genetic.params.{NamedParams, Params}
import genetic.util.JavaUtil
import genetic.util.JavaUtil.{formatDouble, padInt}
...
object Parametric {
  ...
  def join[A](p1: Parametric[Parametric[A]]): Parametric[A] = {
    // Assumes the genetic.params don't depend on each other, i.e. can't be a real monad, and it isn't; it's just an applicative.
