...
case class Parametric[+A](applyParams: (Map[String, Int], Map[String, Double]) => A, intNamesDefaults: SortedMap[String, Int], intsMax: SortedMap[String, Int], doubleNamesDefaults: SortedMap[String, Double]) {
  def map[B](f: A => B): Parametric[B] =
    Parametric[B]((ints, doubles) => f(applyParams(ints, doubles)), intNamesDefaults, intsMax, doubleNamesDefaults)
  ...
  def sub(addSub: String => String, stripSub: String => String): Parametric[A] =
  ...
  def prefixed(prefix: String): Parametric[A] =
    sub(prefix + _, _.drop(prefix.length))
  ...
  def updateArrayParams(p: Params): Parametric[A] = {
    ...
    Parametric(applyParams, ints, intsMax, doubles)
  }
  ...
  def updateDefaults(intNamesDefaults: Map[String, Int],
    ...
    Parametric(this.applyParams, this.intNamesDefaults ++ intNamesDefaults, this.intsMax ++ intsNamesMax, this.doubleNamesDefaults ++ doubleNamesDefaults)
  }

  override def toString: String = s"Default Params: (ints: ${intNamesDefaults.toString.drop(3)}, intsMax: ${intsMax.toString.drop(3)}, doubles: ${doubleNamesDefaults.toMap.toString.drop(3)})"
}
...
object Parametric {
  ...
  def point[A](a: => A): Parametric[A] =
    Parametric((ints, doubles) => a, SortedMap.empty, SortedMap.empty, SortedMap.empty)

  def map2[A, B, C](p1: Parametric[A], p2: Parametric[B], f: (A, B) => C): Parametric[C] = {
    Parametric((ints, doubles) => f(p1.applyParams(ints, doubles), p2.applyParams(ints, doubles)), p1.intNamesDefaults ++ p2.intNamesDefaults, p1.intsMax ++ p2.intsMax, p1.doubleNamesDefaults ++ p2.doubleNamesDefaults)
  }
  ...
  def join[A](p1: Parametric[Parametric[A]]): Parametric[A] = {
    ...
    Parametric((ints, doubles) => p1.applyParams(ints, doubles).applyParams(ints, doubles), p1.intNamesDefaults ++ p2.intNamesDefaults, p1.intsMax ++ p2.intsMax, p1.doubleNamesDefaults ++ p2.doubleNamesDefaults)
  }

  def intParam(name: String, default: Int, maxValue: Int): Parametric[Int] =
